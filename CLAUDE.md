...
- A second `--top-fade-inset` custom property (`0px` by default, `40px` under `@media (display-mode: standalone)`) is added to the top padding only. It exists to push the header row clear of iOS 26+'s Liquid Glass **scroll edge effect** — a system blur painted over the top ~100px of an installed web app's view, above the WebView, which no meta tag, manifest field, or CSS can disable or cover. Content left inside that band renders washed out. Don't remove this inset, and don't try to fix the fade by dropping `apple-mobile-web-app-status-bar-style: black-translucent` (measured ineffective, and it would zero `env(safe-area-inset-top)` and collapse the padding scheme) or by putting a `backdrop-filter`/translucent bar up there (that actively disqualifies the element from WebKit's edge sampling). Tune the `40px` if the fade depth changes; set it to `0px` if Apple ever ships an opt-out.
